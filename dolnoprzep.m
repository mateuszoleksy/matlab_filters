...
%wykresy odpowiadające za odpowiedź "impulsową" filtru FIR: własnego
%algorytmu oraz FIR algorytm matlab
figure("Name", "Odpowiedź impulsowa", "NumberTitle", "off");
subplot(2,1,1);
stem(h,'r-');
title("Odpowiedz impulsowa filtru DP, własny algorytm.");

subplot(2,1,2);
b =  fir1(N-1, fg / (fs/2), 'low');
stem(b);
...
%stosujemy transmitancję filtru dla kazdej probki
original = samples;
...
%obliczamy transformatę odwrotna dla przekształconego widma
plot(t,Y2,'r-');
